...
def create_mobilenetv2_ssd_lite(num_classes, width_mult=1.0, use_batch_norm=True, onnx_compatible=False, is_test=False):
    ...
    regression_headers = ModuleList([
        SeperableConv2d(in_channels=round(576 * width_mult), out_channels=8 * 4,
                        kernel_size=3, padding=1, onnx_compatible=False),
        SeperableConv2d(in_channels=1280, out_channels=8 * 4, kernel_size=3, padding=1, onnx_compatible=False),
        SeperableConv2d(in_channels=512, out_channels=8 * 4, kernel_size=3, padding=1, onnx_compatible=False),
    ...
    classification_headers = ModuleList([
        SeperableConv2d(in_channels=round(576 * width_mult), out_channels=8 * num_classes, kernel_size=3, padding=1),
        SeperableConv2d(in_channels=1280, out_channels=8 * num_classes, kernel_size=3, padding=1),
        SeperableConv2d(in_channels=512, out_channels=8 * num_classes, kernel_size=3, padding=1),
